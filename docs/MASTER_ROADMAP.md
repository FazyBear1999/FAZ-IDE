...
- Slice 4 (2026-02-21): Panel-contract guard — enforced "never close the final remaining primary panel" in `assets/js/app.js` to prevent empty-workspace dead-end states. Validation: focused layout contract in `tests/layout-micro.spec.js`.
- Slice 5 (2026-02-21): Panel API hardening — invalid panel names now no-op safely across open/order/dock/toggle flows in `assets/js/app.js`, preventing accidental layout mutation from bad inputs. Validation: focused invalid-panel contract in `tests/layout-micro.spec.js`.
- Slice 6 (2026-02-21): Panel row/order normalization hardening — malformed docking rows and non-integer order indices now no-op safely in `assets/js/app.js`, preventing invalid-input-driven row churn. Validation: focused malformed row/order contract in `tests/layout-micro.spec.js`.
...
- Slice 13 (2026-02-22): Lesson parser blank-line lookup optimization — `assets/js/core/lessonEngine.js` now precomputes next blank-line indices for `[LESSON:*]` segmentation, removing repeated marker-local scans while preserving step extraction semantics.
- Slice 14 (2026-02-22): Lesson single-completion progression — `assets/js/core/lessonEngine.js` now collapses parsed lesson segments into one typing completion target per lesson session, keeping detailed lesson comments/content while removing multi-step completion checkpoints. Validation: focused lesson contract in `tests/lesson-system-micro.spec.js`.
